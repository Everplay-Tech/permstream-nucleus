...
#ifndef BENCH_REMOVE_PERMSTREAM
extern "C" {
    int32_t permstream_compress_c(const uint8_t *inbuf, size_t insize, uint8_t *outbuf, size_t *outsize);
    int32_t permstream_decompress_c(const uint8_t *inbuf, size_t insize, uint8_t *outbuf, size_t *outsize);
}

int64_t lzbench_permstream_compress(char *inbuf, size_t insize, char *outbuf, size_t outsize, codec_options_t *codec_options)
{
    size_t out_len = outsize;
    int32_t res = permstream_compress_c((const uint8_t*)inbuf, insize, (uint8_t*)outbuf, &out_len);
    if (res == 0 && out_len > 0) return out_len;
    return 0;
}

int64_t lzbench_permstream_decompress(char *inbuf, size_t insize, char *outbuf, size_t outsize, codec_options_t *codec_options)
{
    size_t out_len = outsize;
    int32_t res = permstream_decompress_c((const uint8_t*)inbuf, insize, (uint8_t*)outbuf, &out_len);
    if (res == 0 && out_len == outsize) return out_len;
    return 0;
}
#endif
